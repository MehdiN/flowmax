...
# the sampling function by solving the ODE
@eqx.filter_jit
def single_sample_fn(model,dt0,num_steps,key)->jax.Array:
    def flow(t,x, args):
        return model(x,t)
    term = dfx.ODETerm(flow)
    solver = dfx.Tsit5()
    x0=jxr.normal(key,(2,))
    timegrid = jnp.linspace(0,1,num_steps)
    saveat = dfx.SaveAt(t0=False,t1=False,ts=timegrid)
    sol = dfx.diffeqsolve(term,solver,t0=0,t1=1,dt0=dt0,y0=x0,saveat=saveat)
    return sol.ys
...
def main():
    ...
    sample_fn = ft.partial(single_sample_fn,model,0.1,n_steps)
    samples = jax.vmap(sample_fn)(sample_keys)
    samples = rearrange(samples,"n b d ->b n d")
    fig, axes = plt.subplots(1, n_steps, figsize=(30, 4), sharex=True, sharey=True)
    
    axes[0].set_xlim(-3.0, 3.0)
    axes[0].set_ylim(-3.0, 3.0)
    for i in range(n_steps):
        axes[i ].scatter(samples[i,:, 0], samples[i,:, 1], s=10)
        axes[i].set_title(f't = {timesteps[i]:.2f}')
